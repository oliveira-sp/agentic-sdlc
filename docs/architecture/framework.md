...
The framework preserves human control over important decisions. In
`opencode.jsonc`, permission rules gate the dangerous surface:
...
The global config ships the machinery; each repository carries its own
working agreements, seeded by `setup-engineering`:
...
3. **Safety in configuration.** Approval is enforced by `opencode.jsonc`
   permission rules and read-only subagents, not by convention.
